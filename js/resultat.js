...
$(document).ready(function () {
    ...
    tableClasses.forEach(tableClass => {
        const container = $('body > .cont > .data > .main > div > fieldset.' + tableClass + ' > .customers');

        container.on('click', 'tbody .total-row-prest td .btn.min.first-check a', function (event) {
            console.log('hello');
    ...
    tableClasses.forEach(tableClass => {
        ...
        container.on('click', 'tbody .total-row-collab td .btn.min.first-check a', function (event) {
            console.log('hello 2');
            event.preventDefault();
...
function sortColSelectOption(div) {
    ...
    if (sortCol == undefined || sortCol == null || sortCol == "") sortCol = "year";
...
function sortColAdapt() {
    var code = $("body .cont .data .main div .all .left-div .select.sortAnalyse").attr("code");
    if (code == undefined || code == null) code = "";
    var option = $("body .cont .data .main div .all .left-div .select.sortAnalyse  > .data > .list > .option[code='" + code + "']");
...
function sortTable(tableId) {
    ...
    var prestRowsDelete = table.querySelectorAll(" tr > td > .btn.min.first-check > a > div > i");
    ...
    for (i = rows.length - 1; i >= 1; i--) {
        ...
        if (collab !== currentcollab) {
            ...
            tdWithButton.innerHTML = formBtn({ "key": "first-check", "ico": "fa-circle" });
...
$(document).ready(function() {
    ...
    $('body > .cont > .data > .main > div > .all > .right-div > .affiche-exep > a').on('click', function () {
        var targetElements = $('body > .cont > .data > .main > div > fieldset > .customers > tbody > tr');
    ...
    sortColAdapt();
    $("body .cont .data .main div .all .left-div .select.sortAnalyse .data a").off("click").on("click", function (event) {
        formSelectInit($(event.target).parents(".select"));
    });
    $("body .cont .data .main div .all .left-div .select.sortAnalyse .data .list .input.filter .data input").off("input").on("input", function (event) {
        sortColSelectFilter($(event.target).parents(".select"));
    });
    $("body .cont .data .main div .all .left-div .select.sortAnalyse .data .list .option a").off("click").on("click", function (event) {
